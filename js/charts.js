...
const BL = {
    ...
    font: { family: "Inter,-apple-system,sans-serif", color: "#e6edf3", size: 12 },
    ...
    hoverlabel: { bgcolor: "#161b22", font: { size: 12, color: "#e6edf3" }, bordercolor: "#5eead4" },
    legend: { orientation: "h", y: -0.2, font: { size: 11 } },
    colorway: ["#5eead4", "#58a6ff", "#3fb950", "#bc8cff", "#f85149", "#d29922", "#99f6e4", "#2dd4bf"]
};
...
function chartEngagementLine(el, df) {
    ...
    if (s.length >= 7) {
        const r = y.map((_, i) => { if (i < 6) return null; return y.slice(i - 6, i + 1).reduce((a, b) => a + b, 0) / 7; });
        t.push({ x, y: r, mode: "lines", name: "Media Movel (7)", line: { color: C.blue, width: 2, dash: "dash" }, hovertemplate: "<b>%{x|%d/%m/%Y}</b><br>Media: %{y:.2f}%<extra></extra>" });
    }
    Plotly.newPlot(el, t, ml({ title: { text: "Taxa de Engagement ao Longo do Tempo", font: { size: 15 } }, yaxis: { ...BL.yaxis, title: "Engagement Rate (%)" }, height: 360 }), PC);
}
...
function chartTopPostsBar(el, df) {
    ...
    Plotly.newPlot(el, [
        { y: lb, x: top.map(d => d.likes), name: "Likes", orientation: "h", marker: { color: C.teal }, hovertemplate: "Likes: %{x:,.0f}<extra></extra>" },
        { y: lb, x: top.map(d => d.comments), name: "Comments", orientation: "h", marker: { color: C.blue }, hovertemplate: "Comments: %{x:,.0f}<extra></extra>" }
    ], ml({ title: { text: "Top 10 Posts por Engagement", font: { size: 15 } }, barmode: "stack", height: 380, xaxis: { ...BL.xaxis, title: "Interacoes" } }), PC);
}

function chartEngagementDist(el, df) {
    const r = df.map(d => d.engagement_rate), avg = r.reduce((a, b) => a + b, 0) / r.length;
    Plotly.newPlot(el, [{ x: r, type: "histogram", nbinsx: 20, marker: { color: C.teal, line: { color: "#161b22", width: 1 } }, hovertemplate: "Engagement: %{x:.2f}%<br>Posts: %{y}<extra></extra>" }], ml({
        title: { text: "Distribuicao de Engagement Rate", font: { size: 15 } },
        xaxis: { ...BL.xaxis, title: "Engagement Rate (%)" }, yaxis: { ...BL.yaxis, title: "Posts" },
        shapes: [{ type: "line", x0: avg, x1: avg, y0: 0, y1: 1, yref: "paper", line: { color: C.red, width: 2, dash: "dash" } }],
        annotations: [{ x: avg, y: 1, yref: "paper", text: "Media: " + avg.toFixed(2) + "%", showarrow: false, font: { color: C.red, size: 11 }, yanchor: "bottom" }],
        height: 360
    }), PC);
}
...
function chartHeatmap(el, df) {
    ...
    Plotly.newPlot(el, [{
        ...
        y: daysPt,
        type: "heatmap",
        colorscale: [[0, "#0d1117"], [0.15, "#161b22"], [0.35, "#1c3a3a"], [0.6, "#2dd4bf"], [1, "#5eead4"]],
        hovertemplate: "<b>%{y}</b> as <b>%{x}</b><br>%{z} post(s)<extra></extra>",
        showscale: true,
        colorbar: { title: { text: "Posts", font: { color: "#8b949e", size: 11 } }, tickfont: { color: "#8b949e" } }
    }], ml({
        title: { text: "Padrao de Postagem (Dia x Hora)", font: { size: 15 } },
        xaxis: { ...BL.xaxis, title: "Hora do Dia", dtick: 2 },
        height: 360
    }), PC);
}
...
function chartEngDay(el, df) {
    ...
    Plotly.newPlot(el, [{
        ...
        marker: { color: av.map((_, i) => i === mi ? C.teal : C.tealDk + "88"), line: { color: "#161b22", width: 1 } },
        text: av.map(v => v.toFixed(2) + "%"), textposition: "outside", textfont: { color: "#e6edf3", size: 10 },
        hovertemplate: "<b>%{x}</b><br>Engagement: %{y:.3f}%<extra></extra>"
    }], ml({ title: { text: "Engagement Medio por Dia da Semana", font: { size: 15 } }, yaxis: { ...BL.yaxis, title: "Engagement (%)" }, height: 360 }), PC);
}
...
function chartEngHour(el, df) {
    ...
    Plotly.newPlot(el, [{
        ...
        marker: { color: av.map((_, i) => i === mi ? C.teal : C.tealDk + "88"), line: { color: "#161b22", width: 1 } },
        hovertemplate: "<b>%{x}</b><br>Engagement: %{y:.3f}%<extra></extra>"
    }], ml({ title: { text: "Engagement Medio por Hora do Dia", font: { size: 15 } }, yaxis: { ...BL.yaxis, title: "Engagement (%)" }, xaxis: { ...BL.xaxis, dtick: 2 }, height: 360 }), PC);
}
...
function chartFrequency(el, df) {
    ...
    Plotly.newPlot(el, [{
        ...
        line: { color: C.teal, width: 2, shape: "spline" },
        marker: { size: 6, color: C.teal, line: { color: "#161b22", width: 1 } },
        fill: "tozeroy", fillcolor: "rgba(94,234,212,.08)",
        hovertemplate: "<b>Semana de %{x}</b><br>%{y} post(s)<extra></extra>"
    }], ml({ title: { text: "Frequencia de Postagem por Semana", font: { size: 15 } }, yaxis: { ...BL.yaxis, title: "Posts / Semana" }, height: 360 }), PC);
}

function chartCaptionEng(el, df) {
    Plotly.newPlot(el, [{
        x: df.map(d => d.caption.length), y: df.map(d => d.engagement_rate), mode: "markers",
        marker: { size: 8, color: df.map(d => d.likes + d.comments), colorscale: [[0, C.tealDk], [1, C.teal]], showscale: true, colorbar: { title: { text: "Interacoes", font: { size: 11 } } }, line: { width: 1, color: "#30363d" }, opacity: 0.8 },
        hovertemplate: "<b>Caption:</b> %{x} chars<br><b>Engagement:</b> %{y:.2f}%<extra></extra>"
    }], ml({ title: { text: "Tamanho da Caption vs Engagement", font: { size: 15 } }, xaxis: { ...BL.xaxis, title: "Caracteres na Caption" }, yaxis: { ...BL.yaxis, title: "Engagement (%)" }, height: 380 }), PC);
}
...
function chartHashtags(el, ht) {
    ...
    Plotly.newPlot(el, [{
        ...
        marker: { color: C.teal, line: { color: "#161b22", width: 1 } },
        hovertemplate: "<b>%{y}</b><br>%{x} uso(s)<extra></extra>"
    }], ml({ title: { text: "Hashtags Mais Utilizadas", font: { size: 15 } }, xaxis: { ...BL.xaxis, title: "Frequencia" }, height: Math.max(320, e.length * 28) }), PC);
}
